...
| 2026-09-25 | **第 65 批 · 修 report_bug 的两条失效路径（Round 4 子线工具侧；A/A2）**。

  背景：Round 4 的"模拟用户发帖"动作 `report_bug` 在 `wang`/`lin` 等人格上
  持续产出反馈帖。按巡检累计 231 条后清点，发现两条**方向相反**的失效路径：

  **A · 无素材时编造**（47 条）
  `fb4.mjs` 在 `recentProblems` 为空时的 fallback 文案是
  「这次没撞到具体报错，就说说你刚才用得不顺手的地方」——
  这句话**等于命令模型编一个抱怨**。于是产出了大量平台上不存在的功能抱怨：
  · 「首充送 20 块结果账单没动」→ 平台**没有首充活动**
  · 「手机端点开编辑器、切后台草稿没了」→ 平台是**网页**，没有 App
  · 「导出图片多了个 10 像素白边」→ **没有图片导出功能**
  · 「点右上角三个点才看到编辑资料」→ 个人设置就在**侧栏一级菜单**
  统计：47 条无素材帖里约 4/5 经核实是编造。

  为什么必须杜绝（而不只是"少发几条"）：这类帖会**污染监工信号** ——
  我会照着去查一个不存在的功能（实测为查证 #367「按天图 hover 不出数字」
  花了一整轮，最后发现热区是整个图表、帖子描述不成立）；
  也可能误导真实用户以为平台真有这些毛病。

  **A2 · 有素材时重复取材**（184 条）
  `recentProblems` 数组只增不减（超 20 条才 shift），而 `report_bug`
  每次取 `slice(-3)` 却**不标记已用** → 同一个故障被反复取材。
  实测：「生成到一半…」系列在 35 分钟内产出 12 条同题帖，
  单主题累计 44+ 条，占社区同题帖的绝大部分。
  更关键的是：**故障早已停止，帖子还在继续产出**（取材只用历史条目）。

  为什么这条比编造更隐蔽：这类帖**内容是真的**（每条都能对上真实故障），
  核对时看不出问题；但会把社区淹掉，还会让监看者误以为
  "很多用户都在报同一个问题"（实际只是同一次故障被播报了 44 遍）。

  **修法（两道闸门）**：
  1. 新增 `freshProblems()`：取「尚未被取材过」的条目（`CONSUMED` 指纹集合）；
     发帖成功后才标记（失败下次可重试）→ **每个真实故障最多 1 帖**。
  2. `myProblems` 为空时**直接跳过，不发帖** → 从源头消除"命令模型编造"。
     同时保留可验证性：`FB_NO_MATERIAL=1` 显式走这条分支，
     `FB_REPEAT=N` 可在同一进程内验证"第二次会跳过"
     （闸门② 的标记是进程内的，跨进程测不出真实行为）。

  **验证（真跑，不是只看代码）**：
  ```
  闸门①（无素材）：FB_NO_MATERIAL=1 → 「本轮没有真实问题可报，跳过（不编造）」✓
  闸门②（重复取材）：FB_REPEAT=2 同一进程内跑两次
                    结果[1/2]：已发反馈帖 #955：分组选第一个就废了？（带图）
                    结果[2/2]：本轮没有真实问题可报，跳过（不编造）✓
  ```
  即：第一次发帖、第二次因"素材已消耗"跳过 —— 正是修复目标。
  （验证过程产生的两条测试帖 #953/#955 已置为隐藏 status=2，
    因为那是我跑测的产物、不是子线的真实经历。）

  **工具位置与提交说明**：改动在 `.workbuddy-ai/fb4.mjs`，该目录已进
  `.gitignore`（内含测试服务器口令），所以**不在仓库里**，此处仅作变更记录。

  **教训（与本批第 64 批的 J 同源）**：
  · "内容是真的"不等于"这条反馈有价值" —— 重复 44 遍的真事一样会淹没信号。
  · 测试工具的**fallback 文案**要特别小心：一句为了让流程不中断而写的
    「随便说点什么」，实测会变成"命令模型编造"。

| 2026-09-25 | **第 66 批 · 修探针刷屏（I 项）：给发帖类动作加冷却**。

  **怎么发现的**：A/A2 修好后（第 65 批），report_bug 归零，但复核 I 项时看到
  探针帖占社区比例**反而从 88% 升到 91.5%**。查下来驱动因素是
  `post_community` 在 126 分钟里发了 **79 帖**（5 个子线合计），
  即每个子线平均每 8 分钟发一条。

  **判断**：这不是真人行为，是**模拟失真** —— 模型在"选动作"时对发帖
  没有偏好约束，选中就发。后果有两层：
  ① 社区被探针帖淹没（真实用户打开社区第一屏全是测试内容）；
  ② 让"模拟真实用户"这个目的一并失真（真人发帖频率远低于此）。

  **改法**（`.workbuddy-ai/fb4.mjs`，工具侧）：
  · 新增 `POST_COOLDOWN_MS`（默认 20 分钟，可用环境变量覆盖）
  · `CONTENT_ACTIONS = {post_community, report_bug, read_community, upload_media}`
    —— 只对"会产生持久内容"的动作限流；查看类（看用量/看价格）不受限，
       因为多看几次无副作用
  · `pickAction` 把冷却中的动作**从菜单里剔除**（而不是先选中再否决）——
    后者会浪费一次模型生成，也容易造成选择偏差
  · 动作**执行成功后**才记时刻（失败不该占冷却额度）

  **验证（两层，都真跑）**：
  ① 逻辑层（直接测 onCooldown 的四种情形）：
     初始全部可选 ✓ / 发帖后 post_community 不可选 ✓ /
     未触发的 report_bug 仍可选 ✓（互不影响）/
     查看类 check_usage_log 仍可选 ✓（不受冷却）/
     模拟 11.6 分钟后恢复可选 ✓
  ② 生产层（新 runner 上线后 6 分钟实测）：
     post_community 实际发帖 **1 次**（修复前同口径约 3.8 次/6分钟）
     同时查看类动作正常高频（manage_token 14、check_usage_log 12）
     —— 说明冷却只限制了该限制的，没把子线"冻住"。

  **本轮同时更正了一个我自己的预期错误**：
  此前我在待办里写「I：探针帖占社区 88%（新增速度已下降）」，
  那个"已下降"是**推断而非实测**，实际是上升的。
  错因：把"反馈帖归零"当成了"探针帖增速下降"，
  但反馈帖在总量里占比很小，真正推动占比上升的是闲聊帖。
  （同期还犯了另一次同类错误：第 73 轮把 E 项的证据凭印象写错、
  且是先写结论后验证。两次都已更正，教训归为一条：
  **写日志时若还没查，就写"待复核"，不要写"已复核"**。）

  至此 Round 4 暴露的四个问题全部修复并验证：
  · J  平台侧 content-error 误判（第 64 批，已确证 4 小时零复现）
  · A  工具侧无素材编造（第 65 批，无素材即跳过）
  · A2 工具侧重复取材（第 65 批，每故障最多 1 帖）
  · I  工具侧发帖刷屏（本批，20 分钟冷却）

| 2026-09-25 | **第 67 批 · 修监工面板的假告警（B 项，工具侧）**。

  **现象**：`fb4_watch.mjs status` 每轮多显示 5 条
  「⚠ 已 N 分钟无动静（可能卡住或已退出）」，而那个 N **恰好等于进程运行时长**。
  从第 1 轮巡检起每轮都报，累计约 80 轮。

  **根因**（第 2 轮就定位，一直没修）：
  `fb4.mjs` 的 `emit()` 先写 `persona: PERSONA_KEY`，再展开调用方传入的 data。
  而 `main()` 里发 start 事件时传了 `persona: <人格中文名>` ——
  **正好覆盖掉 persona 键**。于是事件流里多出 5 个以中文名
  （「小周不熬夜了」…）为 persona 的"幽灵"条目，它们只有那一条 start 记录。

  **为什么必须修**（它本身不影响平台）：
  我的巡检规程写着「看到这个告警就去查进程、死了就重启」——
  **照做会去重启 5 个完全健康的子线**。
  一个会误导自己操作的告警，比没有告警更危险。

  **修法（两处，缺一不可）**：
  1. `fb4.mjs:833` —— start 事件的键名 `persona` → `personaName`（修根因）。
  2. `fb4_watch.mjs` 的存活检查 —— **只显示有 round 事件的条目**
     （幽灵条目只有 start、没有 round），并显式打印被忽略的条目名。
     **为什么第 2 处也必要**：事件流是追加的，历史幽灵记录不会消失 ——
     只修 fb4.mjs 能保证"不再新增"，但旧的 5 条仍会每轮显示假告警。

  **验证（两层）**：
  ```
  ① 逻辑层：复现 emit 的键覆盖行为
       旧写法 persona = 阿蓝画不完（被覆盖 → 幽灵项来源）
       新写法 persona = lan（正确）、personaName = 阿蓝画不完  ✓
  ② 生产层：
       · 新 start 事件的 persona 均为 key（zhou/wang/lin/may/lan）
         「仍用中文名作 persona 的新 start 事件: 0 条」✓
       · 幽灵条目本次重启后新增 0 条（最后一条停在 14:39 的上次重启）✓
       · watch 面板：只显示 5 个真实子线，并提示
         「另有 5 个无 round 记录的条目已忽略：…」✓
  ```

  **至此 Round 4 相关的工具侧缺陷全部清理完毕**：
  A（无素材编造）、A2（重复取材）、I（发帖刷屏）、B（假告警），
  加上平台侧的 J（content-error 误判）—— 共 5 项，全部修复 + 验证。

<br>

| 2026-09-25 | **第 68 批 · 补「OD币值锚点」与「分组倍率去哪看」（Round 4 第 92 轮发现，平台侧文案）**。

  **怎么发现的**：第 92 轮巡检按规程查「跨人格困惑」（同一问题被 3 个以上人格提到）。
  两条都过阈值，且**是去重后的真实发言**，不是关键词误命中：
  · **OD币到底值多少**：5/5 人格、6 小时内去重 **330 条**发言。样本：
    - zhou「不知道 OD 币是个啥汇率，绷不住」
    - may「页面只给OD币，不给人民币汇率，0.0088 OD币到底是八分还是八毛」
    - wang「这价格看不出贵不贵，得知道我充值比例才能算」
  · **分组倍率是什么/在哪看**：4/5 人格、去重 **68 条**。样本：
    - zhou「那个『分组倍率』是啥我根本没找着，光有单价没用啊」
    - lan 「两边加完再乘我那个分组的倍率。不过我现在不知道自己的分组倍率是多少」

  **先复现，再定性**（浏览器实测，不是看源码推断）：
  1. `GET /api/token/groups` 确实返回 `rate` 字段；
  2. 建令牌的分组下拉**确实**渲染倍率徽章（实测选项文字 `1 | 仅 deepseek-flash | × | 50`）；
  3. 密钥列表的分组标签悬浮**确实**带倍率（实测绑 rate=50 的密钥悬浮显示
     `分组：1（倍率 ×50）`），但 `rate === 1` 时该段被省略
     （`VendorIcon.jsx:497-500`）—— 符合设计，不是 bug；
  4. 用户侧 6 个页面（`/` `/console` `/pricing` `/log` `/token` `/profile`）
     **渲染后**都没有「美元 / USD / $」任何一个锚点（浏览器读 `body.innerText`）；
  5. 「模型价格」页只写了 `1 OD币 = 10,000 额度` —— 那是**额度单位**换算，不是币值。

  **定性**：这是**解释缺失**，不是「倍率被藏起来」。
  倍率在下拉徽章与悬浮提示里都有；缺的是「它是什么、会乘进账单、你的倍率在哪看」三句话。
  连同 3 处一起补的，还有 `ConsolePage.jsx` 的一处**注释与实现不一致**：
  第 55 批的注释写「OD币就是美元计价的余额」，但落进字符串的只有「余额单位」——
  意图没落地。

  **改法（纯文案，三处，不碰计费逻辑）**：
  | 文件 | 位置 | 改动 |
  |---|---|---|
  | `pages/PricingPage.jsx` | 头部 tags | 追加「（1 OD币 = 1 美元）」 |
  | `pages/PricingPage.jsx` | 计费说明 Alert | 追加指路：「你的分组在『令牌管理』的密钥列表里，悬浮分组名即可看到倍率；没标倍率就是 ×1、不加价」 |
  | `pages/ConsolePage.jsx` | 余额卡 hint | 「余额单位」→「1 OD币 = 1 美元」 |

  **为什么这样改不违反硬约束 #6**（币制只有一条规则，禁引入人民币汇率/其他币名）：
  - 写的是**币制定义句**「1 OD币 = 1 美元」，不是汇率换算；
  - 管理端 `AdminPricingPage.jsx:403` **早就在用同一句话**
    「1 OD币 = 1 美元（1:1 精确核算）」，本批只是把它带到用户侧；
  - 用户看到 1:1 锚点后可自行折算，**不需要**平台提供人民币字段。
  - 未引入任何新依赖、未新增币名、未改 `fmtOd / odOf / unitsPerOd` 链路。

  **验证（门禁全过）**：
  ```
  ① ooapi-web npm run build                         ✓ 27.38s
  ② 产物同步 ooapi-server/web（CSS hash 未变，只换 JS + index.html）✓
  ③ ui-smoke 23 条路由全部 ok，无运行期错误          ✓
  ④ 浏览器复核三处改动（读渲染后的字，非 grep 包）：
     · /pricing 头部：「…1 OD币 = 10,000 额度 （1 OD币 = 1 美元）」        ✅
     · /pricing 说明：「…再乘上你所用分组的倍率。（你的分组在「令牌管理」的
                        密钥列表里，悬浮分组名即可看到倍率；没标倍率就是 ×1、不加价。）」 ✅
     · /console 余额卡悬浮：「共 200.00 OD币。1 OD币 = 1 美元，按每次调用的
                            token 用量扣费（价格见「模型价格」页）…」       ✅
     · 回归：/pricing 表格仍渲染 20 行                                   ✅
  ```

  **本轮值得记下的两个自己的错误**（比结论更有复用价值）：
  1. **口径错误·夸大了影响面**：我在巡检日志里先写了「分组『1』(rate=50) 下 9 个非探针用户，
     真实用户正按 50× 计费」。按用户名前缀逐个核对后：那 12 个账号**全是探针**
     （`zqp*` `zqr*` `probe*` `fb4*`），全站真实账号只有 5 个且都绑 rate=1 分组。
     根因：我用 `user_id NOT IN (71..75)` 代表「非探针」，
     **而 AGENTS.md 第 8 节明明写了 `zqp*`/`zqr*` 是探针** —— 读过却没套用到自己的查询上。
     已在巡检日志追加「口径更正」条目，保留错误痕迹不改原文。
  2. **探针选错元素**：我先用 `innerText` 抓 tooltip，得到「界面没有提示」，
     差点据此判成「倍率被藏起来」。实际是 (a) tooltip 悬浮才渲染，
     (b) 我 hover 的是汇总 chip（调用/消耗/Tokens），**不是表格里的分组 chip**。
     改用真实鼠标坐标后，提示正常。**是探针错，不是功能错。**
     （同类教训第 6 次：先读渲染方式，再写选择器。）

  **另一条副产品**：探针里写了 `SELECT DISTINCT group_name ... ORDER BY id`，
  被线上 `ONLY_FULL_GROUP_BY` 直接拦下（`ER_FIELD_IN_ORDER_NOT_SELECT`）。
  这是**我自己的查询**踩的坑，不是平台 bug —— 仓库文档里早写了这条，
  已改为 `GROUP BY + MAX(id)`。说明该约束在测试机上同样是开着的。

<br>

| 2026-09-25 | **第 69 批 · 修「指路文案在手机上不可执行」+ 监工面板两处显示假象（Round 4 第 93 轮发现）**。

  **怎么发现的**：第 93 轮巡检时，面板「子线发帖」列表出现 5 行 `「undefined」`，
  另有 1134 条显示「落地=未确认」——看着像子线在发空帖、且大批帖子没落地。
  查证后两者**都是面板自己的显示问题**，平台侧没有对应故障：

  · **undefined 标题**：`post_community` 事件共 1139 条，其中 `type=action` 567 条、
    `type=round` 567 条（主循环 `fb4.mjs:857` 发 round 时把 action 一起写进去了）、
    `error` 5 条。round 事件没有 `title`（带的是 `result`），面板按 `e.title` 取 → undefined。
  · **落地=未确认**：面板读的是 `landedTitle`，而现版本 `fb4.mjs:504` 写的是
    `landed` / `landedId`。字段改名后旧字段只在 5 条老事件里存在。
    按新字段重算：**landed=True 562 条、False 0 条** —— 即从未丢过帖子。

  **同一轮还发现了第 68 批（我自己上一批）留下的真缺陷**：
  我写的指路句「悬浮分组名即可看到倍率」**在手机上不可执行**。
  证据链：
  1. 子线读到了这句话并照做，18:29 zhou 原话：
     「倍率还得去令牌管理里悬浮分组名才看得到……**我点了半天没找着**」
  2. 浏览器实测（390px）：令牌列表的「分组」列**整列消失**
     （`TokenPage.jsx:348` 该列为 `responsive: ["lg"]`），
     实测表头只剩「名称/复制/状态/额度/操作」，页面里连「分组」二字都没有；
     而触屏根本没有 hover。
  3. 对照 1440px：分组列在位，但页面**可见文字里也没有倍率**，
     `.oo-rate-badge` 元素 0 个 —— 倍率只存在于悬浮提示中。
  → 结论：桌面靠悬浮可达（实测「分组：1（倍率 ×50）」），手机完全不可达。
  这是我的**文案缺陷**（把用户指向一条移动端不存在的路径），不是平台功能缺失。

  **改法（两处，低风险）**：
  | 文件 | 改动 |
  |---|---|
  | `ooapi-web/src/pages/PricingPage.jsx` 计费说明 | 指路句改成给**两条**路径：「电脑上悬浮密钥的『分组』标签；手机上点『创建令牌』看分组下拉 —— 每个分组后面直接标着 ×N」 |
  | `.workbuddy-ai/fb4_watch.mjs` 帖子落地段 | 只统计 `type === "action"`；标题 `e.title ?? result`；落地改读 `e.landed !== undefined ? e.landed : landedTitle` |

  **为什么手机上那条路可达**：创建令牌的分组下拉里，每个分组右侧本就带
  `GroupRateBadge`（`TokenPage.jsx:70`），实测 390px 下选项文字为
  `1 | 仅 deepseek-flash | × | 50` —— 徽章文字进了 innerText，触屏可点可读。
  所以只需换指路目标，不必新做界面，也不动 `responsive` 列配置。

  **验证**：
  ```
  ① 文案两个视口都读全（1440px / 390px 均含「手机」路径与「×1、不加价」）✓
  ② 桌面路径：悬浮分组标签 → 「分组：1（倍率 ×50）」                        ✓
  ③ 手机路径：创建令牌 → 下拉 3 项，含 ×50 徽章（触屏可达）                 ✓
  ④ 面板修正后：帖子数 1139 → 567（去重），全部「落地=是(id=…)」，无 undefined ✓
  ⑤ ui-smoke 23 条路由全部 ok                                              ✓
  ```

  **本轮第三处自己的错误**（记下来，模式相同）：
  验证脚本里我写 `/×\s*50/` 判断徽章是否存在，得到「❌ 不存在」——
  实际是我把 innerText 的换行替换成了 ` | `，而徽章是**两个 span**
  （`×` 与 `50`），文本形如 `× | 50`，正则匹配不上。
  改用 `/×[\s|]*50/` 后 ✅。**又一次「先怀疑功能、其实错在自己的断言」**——
  这与第 92 轮「hover 错元素」是同一类错误：断言写法比被测对象更容易出错。

<br>

| 2026-09-25 | **第 70 批 · 补 /log（对账页）的币值锚点 —— 第 68 批改动的残余缺口（Round 4 第 94 轮发现）**。

  **怎么发现的**：第 94 轮巡检复核第 68 批的效果，看到困惑密度**大降但没归零**：
  ```
  币值困惑（去重）   轮次    每千轮
   上线前  1000      1353    739.1
   上线后    20       177    113.0     ← 降到 15%，但仍存在
  ```
  密度降了说明改动有效；没归零说明有缺口 —— 于是查剩下这 20 条出在哪。

  **归因（按动作分布，困惑数 / 该动作总数）**：
  ```
  check_usage_log  15/98 = 15%   ← 主缺口（/log 页）
  edit_profile      3/16 = 19%（样本小，与币无关）
  call_gateway      2/38 =  5%
  check_pricing     1/24 =  4%   ← 第 68 批改过的页，已基本解决
  ```
  **93% 的残余困惑（15/16 条）集中在 /log**。样本原话：
  · zhou 18:39「折合人民币我是真算不出来，**页面只给币不给汇率**，服了」
  · zhou 18:43「**OD币到底是啥汇率啊**，服了」
  · may  18:44「我得知道 0.5 个币等于多少钱，才能算这 2582 次调用划不划算」

  **机制定位**（浏览器实测，逐页量「渲染后是否含美元」）：

  | 页面 | innerText 可见 | 仅悬浮可见 |
  |---|---|---|
  | `/pricing` | ✅ 有 | — |
  | `/console` | ❌ 没有 | ✅ 有 |
  | `/log` | ❌ 没有 | ❌ 没有 |
  ...
  关键两点：
  1. **/log 一个字都没有**。实测汇总条可见文字只有
     `调用 14887 消耗 3.6903 OD币 Tokens …` —— 只有数字与币名，**没有单位解释**；
     表头是 `时间/模型/计费/Tokens/…`，「计费」列同样只给数字。
     而 **/log 正是用户对账的那一页**（子线反复说「得翻使用记录才知道扣多少」）。
  2. 第 68 批给 `/console` 加的那句在 `StatCard` 的 `hint` 里 → 走 AntD `<Tooltip>`
     → **只有鼠标悬停才看得见**。子线读页面用 `page.locator("#root").innerText()`，
     抓不到 tooltip 文字 —— 所以 `browse_console` 没进困惑榜，
     **不是因为它没问题，而是因为它们根本没「看见」那句话**。
     （这也解释了为什么 /pricing 有效而 /console 无效：前者在可见文本，后者在悬浮里。）

  **改法（一处，纯文案，改在可见位置）**：
  `ooapi-web/src/pages/LogPage.jsx:490` 消耗汇总 chip
  ```diff
  - <span className="bui-chip" title={`区间消耗（${CURRENCY_NAME}）`}>
  -   消耗 <b>{fmtOd(...)}</b> {CURRENCY_NAME}
  - </span>
  ...
  **为什么写「1 OD币 = 1 美元」而不是「≈ 3.69 美元」**：
  前者是**币制定义句**，管理端 `AdminPricingPage.jsx:403` 早已在用同一句；
  后者是**汇率换算**，会违反全站硬约束（币制只有一条规则，禁新增换算与其他币名）。
  用户看到 1:1 自己就能折算，不需要平台替他算。

  **验证**：
  ```
  ① /log 汇总条可见文字（两个视口都验）：
     1440px：`消耗 3.6903 OD币 （1 OD币 = 1 美元）` ✅
     390px ：同样可见 ✅（子线用 innerText 也读得到）
  ② 窄屏布局回归（F2 同族，必须顺手量）：
     1440px 文档横向溢出 0px、超宽元素 0 个、汇总条 1160×30
     390px  文档横向溢出 0px、汇总条 370×86（正常换行，未被压坏）
  ③ 390px 下唯一超宽元素是 `TABLE w=1351` —— 这是 **F2 早已登记的问题**
     （巡检日志：「F2 窄屏表格缺固定列（/log 1440px 表 1351px 无固定列）」），
     与本次改动无关（改的只是汇总 chip，不是表格）。
  ④ ui-smoke 23 条路由全部 ok ✓
  ```

  **这条的教训（值得复用）**：
  「加了说明」≠「说明可见」。同一句话放在**可见文本**里有效（/pricing 该类困惑
  从 739 降到 113 每千轮），放在**悬浮提示**里等于没加（/console 的锚点，
  用 innerText 根本读不到）。判断一处文案是否生效，必须用**读页面的同一种方式**去量
  —— 这正是我先前「hover 错元素」「正则写错」两次错误的同一根因。

<br>

| 2026-09-25 | **第 71 批 · 修「余额可用 999+ 天」的依据不可见（Round 4 第 95 轮发现，第三次同型缺陷）**。

  **怎么发现的**：第 95 轮按规程查「跨人格困惑」。按 6 小时窗口去重统计：
  ```
  「999+ 天」提及        ：5/5 人格，去重 222 条
  其中带明确困惑表述      ：5/5 人格，去重 161 条
  ```
  远超「3 个以上人格即按缺陷处理」的阈值。样本原话：
  · lan「已用才 3.69，这个我看懂了…看不懂的是『999+ 天 余额可用』，
        这数字哪来的，**我一天就用掉 3.69**，怎么算也算不出四位数」
  · lin「0.05 OD币能撑 999 天？笑死，**这数是不是写死的**」
  · may「已用才 0.15 币怎么可能撑 999 天，估计是按当前消耗速率算的，但那也离谱」
  · wang「我用了 312 次花了 6 分钱，按这速度能撑三年？**不信**」

  **先查显示对不对**（这是关键一步，避免把正确行为当 bug 修）：
  `ConsolePage.jsx:169-177` —— `dailyAvg = 区间消费 / 区间天数`；
  `rawDaysLeft = 余额 / dailyAvg`；超过 999 封顶显示 `999+`。
  按 lan 的账实算：余额 196.31、30 天消费 3.69 → 日均 0.123 → 196.31/0.123 = **1596 天** → 封顶。
  **结论：显示是对的**，不是写死、不是算错。
  真正的毛病是 lan 把「3.69」读成了**日**消费（那其实是 30 天总量）——
  而估算依据 `dailyAvg` **只写在悬浮提示 `hint` 里**，页面上常显的只有「999+ 天」四个字。

  **这是同一类缺陷第三次出现**（前两次证明过的东西，这次又犯了）：
  | 批次 | 把什么放在悬浮里 | 后果 |
  |---|---|---|
  | 第 68 批 | OD币的美元锚点（/console） | 用户看不到；可见版（/pricing）才有效 |
  | 第 69 批 | 分组倍率指路指向手机不存在的悬浮 | 照着做找不到 |
  | 本批 | 余额可用天数的估算依据 | 用户把区间总量当日均 |
  → 结论固化：**解释放悬浮等于没加，必须常显。**

  **改法（一处，纯展示，不动计算）**：
  `ConsolePage.jsx` 余额可用卡片增加 `hintInline`（StatCard 支持常显补充行）：
  ```
  余额充足：按日均 0.1191 OD币 估算已超 999 天     ← 封顶时
  按日均 0.1191 OD币 估算                          ← 未封顶时
  暂无消费，无法估算                                ← 无消费时
  ```
  计算方法、封顶逻辑、文案口径（只用 OD币，不引入汇率）全部未动。

  **验证**：
  ```
  ① 两个视口都用 innerText（子线读页面的同一种方式）量到该句：
       1440px ✅ / 390px ✅
  ② 卡片未变形：1440px 159×77、390px 118×77（多一行没有挤坏并排布局）
  ③ 布局回归：两视口文档横向溢出均 0px、超宽元素 0 个
  ④ ui-smoke 23 条路由全部 ok ✓
  ```

  **同时记录了本轮另一条发现（未动手，属产品决定）**：
  `/console` 的「渠道分布」把渠道编号（`渠道 #42`）暴露给普通用户，且编号对用户
  毫无意义 —— 5/5 人格都提。
  · 复现：普通用户（role=1）可见 `渠道分布 按消费 渠道 #42 0.27 OD币 渠道 #45 0.25 OD币`；
    `GET /api/dashboard/self` 返回 `by_channel:[{"channel_id":42,…}]`（只有 id，无 name）；
    管理员 `/api/dashboard/admin` 才有 `name/type/success_rate`。
  · 与项目自身的边界约定**矛盾**：`LogPage.jsx:85-87` 明确写着「渠道与原始 UA 只对
    管理员可见（渠道等于上游供应商，属于敏感信息）」且实现上 `isAdmin` 才渲染该列；
    `ConsolePage.jsx:5-8` 也写了权限边界原则。
  · 未泄露渠道名/厂商，故不是安全问题，是**展示缺陷 + 边界不一致**。
    两种改法（隐藏卡片 / 换成有意义的厂商信息）已写入巡检日志，等主线程定。

  **另：本轮查证并排除了一个「疑似平台问题」**：
  子线说「有几条补全才 2 tokens 也照样计费」（2 人格提到）。查库：近 24h 补全 ≤2 token
  占 **42.4%（9813/23121）**，全是 `deepseek-v4.1-flash`，且这些调用 prompt 集中在
  **700-1000 tokens**。对照探针 `fb4.mjs` 的 `pickAction()`：发 13 个动作的菜单 + 最近
  6 步历史、**只要求回一个数字** —— 提示词正好几百 token、补全 1-2 token。
  → 是**我探针**「让模型只回一个数字」造成的正常长尾，不是平台计费异常、也不是上游截断。
  子线把它读成「我脚本空转/被截断」属实测者误读。**不修平台，记录以免下次误判。**

<br>

| 2026-09-25 | **第 72 批 · 修「额度」一词撞词（Round 4 第 99 轮发现，5/5 人格困惑）**。

  **怎么发现的**：第 99 轮按规程查跨人格困惑。近 3 小时 **5/5 人格、去重 27 条**在问
  「1 OD币 = 10,000 额度」看不懂，超过阈值。样本原话：
  · may「『1 OD币 = 10,000 额度』这个换算，**跟我实际花的 0.53 币对不上**，
        我得自己拿计算器按一遍才敢信」
  · zhou「OD币是什么币，**1 OD币 = 10,000 额度又是多少 token**，绷不住了」
  · may「计费比例写 1 币等于一万额度，那 199 币到底够我十个人跑多久」
  · lan「『渠道 #45』『1 OD币 = 10,000 额度』这两个我没搞懂是在说啥」

  **机制（浏览器实测，非推断）**：「额度」在同一个页面上被当两个意思用：
  ```
  /console 同页同时出现：
    · 标签「剩余额度 196.31 OD币」/「已用额度 3.69 OD币」
      → 这里「额度」= 余额这件事，**数值单位是 OD币**
    · 说明「1 OD币 = 10,000 额度」
      → 这里「额度」= 内部整数记账单位
  ```
  两处同词异义，用户代入得到「我剩余额度 196.31，可 1 OD币 又等于 10,000 额度」→ 逻辑不通。

  **量化（实测各页「数值单位」分布）**：
  | 页面 | OD币 | 额度 |
  |---|---|---|
  | `/console` | 8 处 | **1 处（就是那句换算本身）** |
  | `/pricing` | 62 处 | 1 处（同上） |
  | `/token` | 2 处 | 0（仅一个列表头叫「额度」，值仍是 OD币） |
  | `/log` | 42 处 | 0 |
  → **用户看到的每个数字都以 OD币 为单位；「额度」从未作为数值单位出现过。**

  **但换算不能删 —— 这点先查清了才动手**：
  实测 `/api/user/self` 返回 `quota = 1963089`、`/api/dashboard/self` 返回
  `account.quota = 1963089`，即**接口确实按原始整数下发**；
  管理端加额度同口径（logs 实测：「补充 5000000 额度给 tester01」）。
  → 换算关系必须保留，问题在于**全站没有一处告诉用户「额度 = 接口里的那个整数单位」**。

  **根因是文案漂移，不是设计如此**：
  规范 `AI协作.md:262` 原文是「`10,000 **额度单位** = 1 OD币`」—— **本来就带「单位」**。
  前端 `odRateText()` 早期把它渲染成「1 OD币 = 10,000 额度」，**把「单位」二字丢了**，
  于是「额度单位」（计量单位）退化成「额度」，与「剩余额度」撞词。

  **改法（一处函数，两个调用点自动生效，纯文案）**：
  `ooapi-web/src/services/format.js:44`
  ```diff
  - return `1 ${CURRENCY_NAME} = ${N} 额度`;
  + return `1 ${CURRENCY_NAME} = ${N} 额度单位（接口返回的整数即此单位）`;
  ```
  调用点：`ConsolePage.jsx:412`（接入信息·计费比例）、`HomePage.jsx:195`（首页换算说明）。
  未改任何数值、未引入新币名、未动 `fmtOd / odOf / unitsPerOd` 链路（符合硬约束）。
  **刻意没做**：没有顺带改「剩余额度 / 已用额度」这两个标签 ——
  那涉及多页改名（ConsolePage / ProfilePage / AdminUsersPage），属产品口径调整，应单独决策。

  **验证**：
  ```
  ① 两个视口 × 两个页面（/console、/）都用 innerText 量：
     · 换算句已含「额度单位（接口返回的整数即此单位）」           ✅
     · 裸「额度」作为单位的出现次数 = 0（改前该处是「10,000 额度」）✅
     · 页面仍保留「剩余额度 / 已用额度」标签（未误伤）            ✅
  ② 布局回归：两视口文档横向溢出均 0px；卡片未被更长的文案挤坏
  ③ ui-smoke 23 条路由全部 ok ✓
  ```

<br>

| 2026-09-25 | **第 73 批 · 补 /profile 余额卡的说明 + 记录一次部署截断事故（Round 4 第 100 轮）**。

  **一、缺陷：/profile 的「剩余额度」卡片，币名与标签并列且无任何说明**（已修）

  **怎么发现的**：第 100 轮跨人格扫描，近 8 小时 **5/5 人格、去重 15 条**发言
  具体指向这张卡的**排版**（比既有登记「OD币 has no hint」更具体）：
  · lin 19:07「『198.98 OD币』这行小字挨着『剩余额度』，我第一反应是**这俩是一个数还是两个数**？」
  · lan 19:37「196.31 孤零零一个，下面并排两个标签『OD币』和『剩余额度』，
        中间也没个分隔，**你说这是币还是额度**。要不就分开写两行。」
  · wang 16:32「到底 OD币 是余额单位还是额度单位，还是这俩是同一个东西」

  **实测（浏览器逐元素读坐标）**：卡片 381×64，文字顺序 `196.31 | OD币 | 剩余额度`；
  文本节点 `196.31`@y=178、`OD币`@y=185、`剩余额度`@y=207；
  **悬浮整卡无任何 tooltip**。
  关键对比：`/console` 上**同一标签**的卡片早在第 68 批已补 hint，`/profile` 没有
  —— 同一种卡片在两个页面行为不一致，这才是它长期被反复问的原因。

  **改法**（`ProfilePage.jsx:79-84`，纯文案，与 /console 对齐）：
  ```
  hint: 这就是你的余额：1 OD币 = 1 美元，按每次调用的 token 用量扣费，用完调用会被拒绝。
  ```
  先回答「这俩是不是同一个东西」，再给币值锚点（1:1 美元；未引入汇率字段，符合硬约束 #6）。

  **顺带修掉一个我自己写出的白屏级隐患**：初版用了 `${CURRENCY_NAME}`，
  但该文件**没有 import 它** → 运行期 `ReferenceError`。
  这正是 AGENTS.md 第 4.2 节点名的白屏成因（「模块级引用未定义」）。
  补上 import 后，验证阶段专门加了 `pageerror` 监听：**运行期错误 0 条**。

  **验证**：
  ```
  ① 两个视口（1440 / 390）悬浮该卡片均出现 hint，且含「1 OD币 = 1 美元」 ✅
  ② /profile #root 文本长度 254（未白屏）、pageerror 0 条                  ✅
  ③ ui-smoke 23 条路由全部 ok                                             ✅
  ```

  **二、部署事故（如实记录，已闭环，线上未白屏）**

  上传 2MB 前端产物时 `qssh.py put` 报 `ConnectionResetError [Win10054]`
  （服务器首连接被重置是已知现象，但这次发生在**传输途中**）。
  回显 "uploaded" 后核对字节数发现服务器侧只有 **990000 / 2013458** 字节
  —— **文件被截断**，而当时 `index.html` 已指向该残缺包。若就此收工即为一次白屏。

  处置顺序（先恢复、再重传、再校验）：
  1. **先回滚** `index.html` 到上一版完好包 `index-5Cbjq78h.js`，跑 ui-smoke 确认 23 路由正常；
  2. 新包按 700KB 分 3 片传输，每片核对字节数（716800 / 716800 / 579858）；
  3. 服务器端 `cat` 合并 → **MD5 与本地完全一致**
     （`60d554f9e8042ec3f42a715ab1b08d23`）；
  4. 安装并切换 `index.html`，`curl` 实测 `js_http=200 size=2013458`；
  5. ui-smoke 全部通过；浏览器实测 hint 正常、运行期错误 0 条。

  **教训**：**上传后必须校验大小/MD5，不能信 "uploaded" 回显。**
  本轮能在 10 分钟内闭环，正是因为核了字节数；跳过这一步就是白屏上线。
  （这与 AGENTS.md「先构建 → 在旧进程上跑 ui-smoke → 通过 → 再走更新流程」
   的顺序要求同源：每一步都要有独立证据，不能靠回显。）

<br>

| 2026-09-25 | **第 74 批 · 「额度单位」补币值锚点 —— 第四次同型缺陷（Round 4 第 104 轮发现）**。

  **怎么发现的**：第 104 轮巡检查跨人格困惑，近 3 小时 **4 人格**反复问同一件事，
  且**引用的正是第 72 批我新写的文案**（说明文案被读到了，但答非所问）：
  · wang 20:52「写了 1 OD币 = 10,000 额度单位，**可没说 10,000 单位对应多少人民币**」
  · lin  20:43「模型价格和『OD币 = 10,000 额度单位』**到底怎么换算成钱**，翻半天没看到」
  · lan  21:01「看不懂的是『OD币』和『额度单位』到底怎么换算」
  · zhou 21:06「那个『1 OD币 = 10,000 额度单位』，看了两遍也没想明白跟我论文有啥关系」

  **实测定位**（浏览器读 /console 渲染后文字）：「计费比例」这一格显示的是
  ```
  1 OD币 = 10,000 额度单位（接口返回的整数即此单位）
  ```
  有「额度单位是什么」的说明，**但没有「它值多少钱」** ——
  而币值锚点（1 OD币 = 1 美元）当时只存在于**同一页余额卡的悬浮提示**里。

  **判定**：这是**同类缺陷第四次复现**（前三次：币值锚点、倍率指路、余额可用依据）：
  **把解释放在悬浮/别处，等于没有解释。**
  第 72 批我讲清了「单位是什么」，但没在同一格回答「值多少」——改的是措辞，不是用户缺的信息。

  **改法**（`ooapi-web/src/services/format.js:62-63`，一处函数、两个调用点自动生效）：
  ```diff
  - return `1 ${CURRENCY_NAME} = ${N} 额度单位（接口返回的整数即此单位）`;
  + return `1 ${CURRENCY_NAME} = ${N} 额度单位（接口返回的整数即此单位；1 ${CURRENCY_NAME} = 1 美元）`;
  ```
  调用点：`ConsolePage.jsx:412`（接入信息·计费比例）、`HomePage.jsx:195`（首页换算说明）。
  写「1 OD币 = 1 美元」而非人民币数额：前者是**币制定义句**
  （管理端 `AdminPricingPage.jsx:403` 早已在用同一句），后者是汇率换算，违反硬约束。
  **刻意没做**：没有再改「额度单位」的措辞 —— 它已被证明可读（多处被原文引用），
  缺的只是同格里的币值，改动最小化。

  **验证**：
  ```
  ① 两个视口 × 两个页面（/console、/）都用 innerText 量到：
     `1 OD币 = 10,000 额度单位（接口返回的整数即此单位；1 OD币 = 1 美元）` ✅
  ② 布局回归：两视口横向溢出均 0px；运行期错误 0 条                 ✅
  ③ ui-smoke 23 条路由全部 ok（部署后重跑）                        ✅
  ④ 发布产物按第 100 批的教训做了**分片上传 + MD5 校验**：
     服务器 `md5sum` = 本地 `9c2b2e6e06870026156f11869d50e600`，字节数 2013479 一致；
     `curl` 实测 `served=200 size=2013479`（完整，非截断）
  ```

<br>

| 2026-09-25 | **第 75 批 · 修「素材行时间措辞歧义 + 模型补造细节」（Round 4 第 108 轮，工具侧）**。

  **背景**：第 104 批给 `report_bug` 加了素材保鲜（`STALE_MS`）并要求素材行带「N 分钟前」，
  目的是让模型知道"这是多久前的事"。**该闸门本轮验证有效** ——
  21:57-22:06 新增的 3 帖取材时刻距各自事件 1.5 / 3.1 / 9.6 分钟，均在 10 分钟窗口内。

  **但引入了两种叙述失真**（本轮实测发现）：
  · lin 帖 #1117：「**等了八分钟**，页面一直转」「我点了**三次重试**，三次都这德行」
    —— 事件 21:51:26、发帖 21:59:26，相差 **8 分钟**；素材行写的正是「8 分钟前」。
      **模型把"事件发生距取材的间隔"读成了"我自己等待的时长"。**
      而"三次重试"素材里完全没有（同期该用户只有 4 条失败记录）。
  · wang 帖 #1119：「等了十来秒」「同一条 prompt 我**重试两次**都这样」—— 同样无素材依据。

  **根因**：
  1. `8 分钟前` 语义有歧义（可读作"事件发生在 8 分钟前"，也可读作"8 分钟前我开始等"）；
  2. 我只补了素材，**没有禁止模型补充素材里没有的细节**（重试次数、等待时长、字符数）。

  **改法（`fb4.mjs` 两处，工具侧）**：
  1. 素材行改为带显式括注，消除歧义：
     ```
     - 发生在约 8 分钟前（这是问题发生的时间，不是你等待了多久）；模型没返回正文：…
     ```
  2. 发帖提示词补反向约束：
     ```
     只说你**确实从上面素材里看到的**：不要编造重试次数、等待时长、具体字符数等
     素材里没有的细节。不确定的一律不写。
     ```

  **为什么值得修**：这类"看起来具体"的编造（三次重试、八分钟）最容易被读帖人当真，
  会让一条**真实故障**的报告掺入不可核实的细节 —— 与闸门①（无素材不发）是同类问题的轻量版。

  **验证**：`node --check` 通过；两处副本 md5 一致（`65e9771c3585d7ba0581513f9b52e805`）；
  重启 5 个子线生效（重启后 5/5 正常，服务 active、health=200）。

  **这条的教训（值得复用）**：修一个缺陷时可能引入另一种失真。
...
| 2026-09-26 | **第 76 批 · 假人全灭复盘 + 车队 24/7 化（守护/浏览器闸门/拟真资料）+ 代码审查一个 P1**。

  **一、「假人为什么只剩一个在真实跑」—— 复盘结论（三层）**

  1. **直接原因：12:01 硬重启后没有任何东西把子线拉回来。**
     fb4_launch.sh 是 nohup 裸启动 —— nohup 只解决「退出终端还在」，
     不解决「机器重启还在」。重启后车队归零，无人补位。
  2. **前因：10 人格并发 + 部署构建把 2 核 / 3.5GB 的机器压死了。**
     02:37 扩到 10 个人格（每个动作各开一个 Chromium、无全局上限），load 冲到 39
     （fb4_launch.sh 顶部注释记录的同一事故）；chen/xiaoyu/laoliu/tina 于 03:00 停摆
     （NO_CHANNEL 冷却期反复报错后被 stop，未再拉起）；11:47 部署 a90821c 的构建
     叠加在 4 个人格之上，**11:52 整机僵死**（journal 戛然而止），12:01 被硬重启。
  3. **中间态：lan（04:42）/ may（08:48）死因无日志痕迹**（事件流无 fatal/end），
     判定为被外部 kill 或 OOM（上个 boot 的内核日志已随重启丢失）。
     12:13-12:16 主线监工会话重新拉起 10 人格并完成头像（DiceBear，10/10），
     但仍无守护 —— 再重启还会全灭。

  **二、本次改动（全部在 /root 工具侧，未动生产仓库与 .env）**

  1. **fb4.mjs 加浏览器全局并发闸门**（跨进程，`/root/fb4/bslots/` 槽位文件）：
     `openBrowser()` 前抢槽（O_EXCL），`b.close()` 时还槽；默认 `FB_BROWSER_SLOTS=2`。
     槽位文件带 pid+时间戳，抢占时清「pid 已死 / 超 10 分钟」的陈旧槽 —— kill -9 不留死占。
     为什么：人格数与「同时开着的 Chromium 数」此前完全无约束，10 个人格就是这次
     load 39 僵死的元凶。有了闸门，**人格总数 10 与浏览器并发 2 解耦**，24 小时常驻才安全。
     另：`ensureAccount` 的登录失败改为 429/5xx 退避重试（20s/40s/60s），
     避免重启后 10 个人格挤进限流窗口时反复 fatal 循环。
  2. **新增守护：/root/fb4_supervisor.sh + systemd `fb4.service`（已 enable）**。
     60s 一轮查缺补漏（同人格 10 分钟重启间隔防崩溃风暴，启动错峰 20s 防限流踩踏）；
     大脑密钥探活结果缓存（失效 15 分钟退避再探）；额度看护（余额 < 10 OD 时走
     管理员 API 补 100 OD，每日每号限一次）；events.jsonl > 200MB 轮转、截图留 2 天。
     单元要点：`KillMode=process`（重启守护不陪葬车队）、`OOMScoreAdjust=-500`
     （内存紧张时先杀子线，守护必须活着补位）、`Environment=ADMIN_USERNAME=odi`
     （管理员用户名在 DB 里是 odi 不是 root；按硬约束不动 .env，放单元文件里）。
     **这就是本次事故的直接解：机器再重启，fb4.service 会自动恢复整支车队。**
  3. **拟真资料 10/10**：display_name / bio / location 走 `PUT /api/users/self` 补齐
     （fb4zhou → 小周不熬夜了 / 湖北武汉…… 全量见 /root/fb4/profiles_assign.mjs 的 PLAN）。
     头像由主线监工会话完成（DiceBear 二次元/欧美卡通，media 2049-2058）。
     旧 bio 的「不是真人」声明按用户本次「更加真实拟真」的要求移除；
     平台层面的披露保留在社区公告（#295 Round 4 模拟用户测试，全站可见）。
  4. **验证**：13:12 实测 10/10 进程在跑新代码（md5 与 /root/fb4.mjs 一致）、
     槽位正常占用、事件流 2 分钟 28 条、`fb4.service` active、
     `/api/status` 200、公开主页已展示新昵称/头像/bio；负载从峰值 32 回落到 < 15 并继续下行。

  **三、顺带踩坑（值得记住）**

  · **路由挂载表**：`/api/user`（单数）挂的是 auth.js（登录/注册）；
    `/api/users`（复数）才是 user.js（个人中心 + 管理）——
    资料更新是 `PUT /api/users/self`、管理员补额度是 `POST /api/users/:id/quota`。
    第一次全写成了单数路径，拿回 Express 的 HTML 404（`Cannot PUT /api/user/self`），
    而登录接口又是通的 —— 症状很有迷惑性。
  · 滚动替换进程时 `pkill -f` 的 ERE 对一部分进程不生效（原因未查明），
    按 `ps -eo pid,args` 的精确字段取 PID 再 kill 更可靠。

  **四、代码审查（仓库只读）发现的问题**

  1. **P1（当时在线上实测稳定 500，现已闭环）**：`routes/buildlog.js` 从 logs 表查
     `cost_units`，该列只在 chat_sessions 表存在 → 线上 `/api/buildlog` 稳定 500
     （12:05 实测复现）。前端 BuildLogCard 失败即隐藏自身（`.catch(() => {})`），
     故障完全静默 —— 这正是 AGENTS.md 点名的「静默吞错误」反模式，连验证门禁都照过了。
     主线监工 12:09 在服务器上用 `quota AS cost_units` 修掉（实测 200），随后已提交进仓库
     （84e6675，含「本地建表名与线上不一致」的踩坑记录）—— 仓库与线上恢复一致，
     我审查时担心的「修复未入库、下次部署会回退」风险随之解除。
  2. **P2**：logs 表 `token_name` 无索引，buildlog 的 `LIKE 'fb4%' OR LIKE 'cc%'`
     每次全表扫描；该接口是公开的且前端 15s 轮询，修好 P1 后这会成为慢性负载。
     建议：给 token_name 加前缀索引，或按 token_id 白名单查询。
  3. **P2**：`LIKE 'fb4%' / 'cc%'` 是前缀匹配，而令牌名由用户自起（仅限长度 64）——
     普通用户把令牌叫 `cc-xxx` 就会让自己的调用流（模型/tokens/花费/耗时）
     被公示接口聚合展示。建议改为在 buildlog_state 里维护精确令牌名单。
  4. **P3**：`.update-stamp.json` 在 12:0x 核对时停在 e9a23d9（a90821c 的 update/apply
     被 11:52 僵死打断，文件落盘但流程未走完）。主线随后完成了 84e6675 / 3406a1c 的
     补部署（门禁与 bundle MD5 校验见其 12:35 批次记录）—— 部署完整性以该记录为准，
     此处留档的是「部署中途僵死会造成 stamp 与实际运行代码漂移」这一形态。

  **五、与主线监工会话的并行协同（如实记录）**

  本批执行期间，另一会话（主线监工，cc* 修复进程）在同一台服务器上并行作业：
  拉起 10 人格、补头像、跑 ui-smoke、执行 /root/cc-tasks 的 T 系列任务。
