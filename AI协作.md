...
**第 36 批 · 媒体库剩余项**（后端与对话链路已完成，缺前端）：
- [ ] 前端媒体库页面（宫格列表、用量条、删除、管理员 `?user_id` 视图）
- [ ] 前端改用新上传接口：`ChatPage` 目前仍发 `dataUrl`（后端会转存媒体库，
      功能可用但多一次 base64 编解码），建议改为「先 `POST /api/media` 拿 media_id → 只传 id」
- [ ] 用户头像上传 UI（ProfilePage，含 Canvas 裁剪压缩）
- [ ] nginx `client_max_body_size`（默认 1MB 会让上传 413，**部署前置项**）
- [ ] 历史数据迁移（把老消息里的内联 base64 转成媒体库文件，可选，不阻塞）
