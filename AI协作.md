...
- [ ] **`temperature=0` 不保证确定性**：同一请求发 3 次，`reasoning_len` 分别是
      54/47/62。上游没传下去或上游不支持 —— 对需要可复现输出的场景有影响。
- [ ] **`-thinking` / `-search` 后缀是空壳**：`hy3` vs `hy3-thinking` vs `hy3-search`
