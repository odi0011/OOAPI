...
**A. 站点与用户体系**
- [~] 用户个人信息类：头像上传、昵称、签名/简介 **已完成**；
      **个人主页（含历史与统计展示）独立页面**仍未做（当前在「个人设置」里维护）——
      之所以整体不算完成，是因为「个人主页」是用户明确列出的一项
- [x] 媒体库：统一文件存储（对话上传的图片/文档、用户头像、社区发帖图），
      表结构、去重、配额、引用计数、清理策略均已实现；
      **后端 + 对话链路 + 前端页面全部完成**（社区发帖图待社区功能落地后接入）
- [ ] 数据看板页面重构（个人维度 + 管理端维度）
...
**第 36 批 · 剩余项**：
- [x] 前端改用新上传接口：`ChatPage` 已改为「先 `POST /api/media` 拿 media_id → 只传 id」
      （上传失败仍回退 dataUrl，不让存储故障阻断发消息）
- [x] nginx `client_max_body_size` —— **复核后确认线上早已配置 32m**
      （`/etc/nginx/sites-available/ooapi`，80 与 443 两个 server 块都有），
      本条目此前登记有误，现更正为已完成
