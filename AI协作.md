...
### 第 36 批：用户明确列出但**尚未实现**的功能（禁止当已完成）

> 用户在最近两次消息里明确要求的功能，目前**大部分未做**。
> 这份清单是「欠账」不是「计划」—— 完成前不得宣称整个系统已完成。

**A. 站点与用户体系**
- [ ] 用户个人信息类：头像上传、昵称、签名/简介、个人主页（含历史与统计展示）
- [ ] 媒体库：统一文件存储（对话上传的图片/文档、用户头像、社区发帖图），
      需要统一规范（表结构、去重、配额、引用计数、清理策略）——**后续很多功能依赖它**
- [ ] 数据看板页面重构（个人维度 + 管理端维度）
- [ ] 系统设置：外观设定更细致（主题色/圆角/字体/布局/背景）；
      站点设定参考 newapi 的做法

**B. 社区**
- [ ] 社区大厅：发帖、评论、点赞、收藏、关注、话题
- [ ] 实时聊天：群聊、单聊、讨论组（SSE 长连接，可复用现有 SSE 经验）
- [ ] 小游戏 / 联机游戏（娱乐模块）

**C. 对话与智能体**
- [ ] 智能体沙盒：调研 bigmodel managed agents 的形态
      （https://docs.bigmodel.cn/cn/managed-agents/quickstart），
      预装沙箱/运行环境策略、能否多模型共用

**D. 商业化与治理**
- [ ] 权限划分明确（用户/管理员/超管的可见范围与可操作范围）
- [ ] 管理员侧更细颗粒度的设定与管理
...
- [x] 媒体库（统一文件存储）—— 后端完成并端到端验证；
      前端媒体库页面（列表/宫格/用量）**未做**，见下方待办
- [ ] 用户个人信息类（头像上传裁剪、昵称、简介、个人主页）
      —— 后端字段与头像接口已就绪，**前端 UI 未做**
- [ ] 社区大厅（发帖/评论/点赞/收藏/关注/话题）
- [ ] 社区实时聊天（群聊/单聊/讨论组）
- [ ] 小游戏 / 联机游戏
- [ ] 数据看板重构
- [ ] 系统设置：外观细化 + 站点设定细化
- [ ] 智能体沙盒（bigmodel managed agents 调研）
- [ ] 权限划分与管理员细颗粒度设定

**第 36 批 · 媒体库具体剩余项**（后端已可用，缺前端与接入）：
- [ ] 前端媒体库页面（宫格列表、用量条、删除、管理员 `?user_id` 视图）
- [ ] 对话链路接入：`chat.js` 目前仍把 base64 写进 `chat_messages.parts`，
      需改为「先存媒体库 → parts 只存 media_id → 渲染时用签名 URL」
      （**这是媒体库的主要价值，未接入就等于问题还在**）
- [ ] 对话删除链路接引用释放（删会话/回退/批量删/删用户时要 `releaseRefs`）
- [ ] 用户头像上传 UI（ProfilePage，含 Canvas 裁剪压缩）
- [ ] nginx `client_max_body_size`（默认 1MB 会让上传 413，部署前置项）
