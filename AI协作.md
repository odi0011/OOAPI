...
### 第 37 批遗留（本轮新增模块引入，不影响当前使用）

> 这些都是第 37 批实现时明确的取舍或已知限制，**不是遗漏**。
> 按「单机单实例」部署均无影响；上多实例或用户量上来时需要处理。

- [ ] **实时推送是进程内实现**（`services/realtime.js`）：多实例部署时
      同一用户连到 A 实例、消息从 B 实例发出就收不到。改法：换 Redis
      pub/sub 或加实例维度聚合。**上多实例前必须处理**。
- [ ] **游戏房间没有清理任务**：`abandoned`/`finished` 的房间会一直留在库里。
      当前量级无感（每局一行），但应加「N 天前的已结束对局归档/删除」的定时任务。
      做法可参考媒体库的 `scheduleMediaCleanup`。
- [ ] **观战是「按需拉取」而非实时**：观战者看到的是打开那一刻的状态，
      需要手动刷新。若要真实时，把对局广播也推给观战者列表（需维护观战者集合）。
- [ ] **`game_records` 表已不再写入**：单机游戏下线后这张表只用于兼容老数据，
      看板与个人主页的「游戏局数」已改为统计 `game_rooms`（联机对局）。
      确认无老数据依赖后可删表。
- [ ] **聊天消息无全文检索**：目前只能按会话翻页，没有跨会话搜索。
      量大时建议加 `chat_room_messages.content` 的全文索引或独立检索表。
- [ ] **社区通知未做**：别人评论/点赞/关注了你，没有任何提醒（只有聊天有未读）。
      需要一张 `notifications` 表 + 复用 SSE 推送；属于下一个功能批次的范围。
- [ ] **帖子的 `media_ids` 只支持图片**：视频/音频已在媒体库的类型白名单里
      （kind 判定支持），但发帖入口只开放了图片选择。需要前端放开 + 体积校验。
- [ ] **外观设置的背景底纹在超宽屏下的平铺密度**未做响应式微调
      （24px 网格在 4K 下显得偏密）。当前只影响观感，不影响可读性。
...
4. **验证行为（强制，不可跳）**：
   ...
   BASE=http://127.0.0.1:3001 xvfb-run -a node tests/ui-smoke.mjs   # 22 个路由逐页：白屏/运行期错误
   node tests/monitor-smoke.mjs                                    # 接口结构断言（22 项）
   node tests/sql-compat.test.mjs                                  # 新 SQL 打到真实库（19 项）
   BASE=http://127.0.0.1:3001 node tests/e2e-modules.mjs           # 社区/聊天/游戏/主页/看板（70 项）
...
| 2026-09-20 | **第 37 批（七）· 历史数据迁移 migrate6**。
  ...
  「被引用」状态，既回收不了、用户也删不掉）。 |
| 2026-09-20 | **第 37 批 · 测试与验证方法**（新增 4 个测试文件，共 81 项新断言）。
  ...
  · `tests/sql-compat.test.mjs`（19 项）：把新模块所有带 GROUP BY/子查询的语句
    ...
    会显式提示「测不出问题」，避免假绿）；
  · `tests/e2e-modules.mjs`（70 项）：社区/聊天/游戏/个人主页/看板/权限分层的
    完整链路，断言到「数据库里的行变了」这一层；
  · `tests/e2e-games-browser.mjs`（22 项）：真实点击，验证六款游戏渲染、
    落子真实生效并落库、海战棋迷雾不泄露、键盘不劫持；
  · `tests/migrate6.test.mjs`（11 项）。
  线上部署后全绿（另含既有 ui-smoke 22 页 / monitor-smoke 22 项）。
