...
//
// 这个值被改过三次，每次的教训都值得留着：
//   ① `deepseek-chat` —— DeepSeek **官方已停用**的旧 id，照抄必 503。
//      四个人格先后撞上（「这是最伤新手的一条」「我自己就来回改了半小时」）。
//   ② `deepseek-flash` —— 我改成这个，**但改错了目标**：它在厂商登记表里存在、
//      也有渠道声明它（渠道「DS手机号登录Key」），可那个渠道不可用 →
//      照抄仍然 503。人格实测原话：「换版之后还在，照首页抄的人第一步就卡死」。
//      **教训：改示例模型名不能只看「名字是否合法」，要看「在这套部署里是否真能调」。**
//   ③ 现在用 `deepseek-v4.1-flash` —— 当前环境下多个启用渠道（workbuddy / OpenCode）
//      都声明且实测 200。
//
// 首页是**未登录可见**的营销页，拿不到「该用户可用模型」（需要登录态），
// 所以这里只能给一个静态名。为了不再出现「示例跑不通」的死循环，
// 示例旁**必须**带一句指路（见下方 CODE_EXAMPLES 的渲染处：
// 「登录后控制台会按你的分组显示真实可调用的模型」）。
const SAMPLE_MODEL = "deepseek-v4.1-flash";
              ...
              {terminalTab === "sdk" && (
                <div className="hr-terminal-body hr-sdk-body">
                  ...
                  <div className="hr-sdk-footer-note">
                    提示：代码示例中的 <code>baseURL</code> 已自动匹配当前站点端点；调用前请将{" "}
                    <code>sk-your-token</code> 替换为控制台生成的实际应用令牌。
                    <br />
                    {/* 这段指路是必须的：示例里的模型名是**静态**的（首页未登录、
                        拿不到"该用户可用模型"），而各部署环境的分组与渠道配置不同 ——
                        若示例模型名在你这里不可用，照抄会拿到 503
                        「当前没有可服务模型…的账号」而不知道下一步（人格实测反馈：
                        用户会陷在「改名字 → 还报错」的死循环里半小时）。
                        控制台（登录后）显示的是**按你的分组算出来的真实可调用模型**。 */}
                    示例里的 <code>model</code> 是通用名。登录后请以
                    <b> 控制台 → 数据看板 → 接入信息 </b>
                    里为你显示的模型名为准（那是按你账号可用分组算出来的，一定能调通）；
                    也可用 <code>GET /v1/models</code> 列出当前密钥可调用的全部模型。
                  </div>
                </div>
              )}
