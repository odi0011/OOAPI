...
export function parseAuthJson(raw) {
  ...
  return {
    access_token,
    refresh_token,
    expires_at: Math.floor(Number(t.expiresAt || t.expires_at || j.expires_at || 0) || 0),
    email: String(j.email || t.email || "").trim(),
    account_uuid: String(j.account_uuid || j.accountUuid || "").trim(),
    organization_uuid: String(j.organization_uuid || "").trim(),
  };
}

/** 刷新登录态（JSON 请求，官方 CLI 同款头） */
...
export async function chat({
  ...
  const resp = await fetch(`${API_BASE}/v1/messages?beta=true`, {
    method: "POST",
    headers: {
      ...
      "x-client-request-id": identity.sessionId, // 每请求不同即可，这里用稳定种子避免噪音
    ...
    if (type === "message_delta") {
      ...
      if (Number.isFinite(outTokens)) {
        usage = {
          ...
          total_tokens: (usage?.prompt_tokens || 0) + outTokens,
...
export async function verify(channel) {
  ...
  const token = await ensureToken(channel);
  if (!token) throw Object.assign(new Error("渠道未配置 Claude 凭据"), { code: "CHANNEL_AUTH_EXPIRED" });
  const identity = claudeIdentity(channel);
  const body = {
    model: channel?.other?.test_model || "claude-haiku-4.5",
    max_tokens: 1,
